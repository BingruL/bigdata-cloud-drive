...
- **公开链接**：文件主可创建公开下载链接，支持过期时间、可选提取码、下载次数统计和撤销；公开下载页 `/s/<token>` 通过 fetch+blob 触发原生下载，服务端错误以提示形式留在页面
- **删除**：软删除，仅在 HBase 元数据打上 `deleted` 标记，HDFS 文件保留；删除文件夹会递归处理子目录和文件，前端会先调 summary 接口提示子项数量与体积
- **回收站**：文件夹与文件分两节展示，文件夹只列顶层删除（被父级连带删除的子项隐藏，避免「先恢复父级」的报错），支持 **恢复** 或 **彻底删除**（后者才真正清理 HDFS）
- **最近访问**：聚合操作日志中 `download` / `preview` 事件，按最近访问时间排序
- **搜索**：搜索框非空时切到全站搜索（`/api/files/search`，跨目录），清空搜索框回到当前目录浏览；同时支持类型筛选与时间范围
- **批量操作**：文件列表支持多选 → 批量下载 / 批量移动（弹出目录树选择器） / 批量移至回收站
...
| GET  | `/api/folders/tree` | 当前用户全部活跃文件夹（前端目录树选择器使用） |
| GET  | `/api/folders/<id>/summary` | 子树规模（文件夹数 / 文件数 / 总体积），删除前确认提示用 |
| GET  | `/api/folders/trash` | 回收站中的顶层文件夹（被父级连带删除的子文件夹不展示） |
...
当前核对输出：`133 passed, 1 warning in ~6s`。完整说明见 `tests/` 目录下的各 `test_*.py`。
